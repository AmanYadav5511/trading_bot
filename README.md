...
- Invalid CLI input (bad symbol, side, order type, quantity, or missing price/stop-price) is caught and reported clearly before any API call is made.
...
- This application is configured exclusively for Binance Futures Testnet and is not intended for live trading.
- Quantity is assumed to be in the base asset unit (e.g., BTC for BTCUSDT).
- MARKET, LIMIT, and STOP order types are supported.
- GTC (Good-Til-Cancelled) is used as the default timeInForce for LIMIT and STOP orders.
